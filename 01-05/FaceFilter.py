...
nose_image = cv2.imread("./output_image.png")
_, frame = cap.read()
...
while True:
    ...
    for face in faces:
        ...
        # Checking if landmarks are within the image boundaries
        if all(0 <= pt.x < cols and 0 <= pt.y < rows for pt in landmarks.parts()):
            # Nose coordinates
            ...
            nose_width = int(hypot(left_nose[0] - right_nose[0], left_nose[1] - right_nose[1]) * 7)
